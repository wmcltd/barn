...
class Gallery_utils
{
  ...
  public static function Getdirfiles($path,$recursive)
	...
	public static function Getgalleryinfo($path)
	{
		...
		$db = cmsms()->GetDB();
		$query = "SELECT g.*, gp.hideparentlink, gp.editors, gt.templateid, gt.template, gt.thumbwidth, gt.thumbheight, gt.resizemethod, gt.maxnumber, gt.sortitems
	...
	public static function CheckEditor($userid, $gid, $editors = array() )
	{
		...
		if ( !$mod->GetPreference('use_permissions') ) return true;
		if ( !empty($editors) && in_array($userid, $editors) ) return true;
		if ( $mod->CheckPermission('Gallery - Edit all galleries') ) return true;
