...
class AlbumContent extends CMSModuleContentType {
	...
	function Show($param = 'content_en')
	{
		if ($param == 'content_en')
		{
			...
			global $id;
			global $params; // I guess this is a hack
			$params['albums'] = $this->GetPropertyValue('albums');
			$params['template'] = $this->GetPropertyValue('album_template');
			$newalbum = new Album();

			//Buffer all output from the Album module and return it
			@ob_start();
			$newalbum->DoAction('default', '', $params, isset($variables['content_id']) ? $variables['content_id']: '');
	...
	function FillParams(&$params)
	{
		...
		if (isset($params))
		{
			...
			$this->mActive = (isset($params['active']));
			
			$this->mShowInMenu = (isset($params['showinmenu']));
